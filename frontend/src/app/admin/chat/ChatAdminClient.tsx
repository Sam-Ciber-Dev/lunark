...
interface Props {
  userId: string;
  userName: string;
  /** When true, the chat renders inline (no fixed top-bar, no `fixed` layout) so it can be embedded inside a tab. */
  embedded?: boolean;
}

export default function ChatAdminClient({ userId, userName, embedded = false }: Props) {
  ...
  return (
    <div
      className={cn(
        ...
        embedded
          ? "h-[calc(100vh-14rem)] min-h-[28rem] overflow-hidden rounded-2xl border border-border"
    ...
    >
      ...
      {/* ── Embedded toolbar (clear only) ─────────────────── */}
      {embedded && (
        <div className="flex items-center justify-end border-b border-border bg-card/50 px-3 py-2">
          <button
            type="button"
            onClick={() => setConfirmClear(true)}
            disabled={messages.length === 0}
            title="Limpar histórico"
            aria-label="Limpar histórico"
            className="flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-[11px] font-medium text-muted-foreground transition-colors hover:text-destructive disabled:cursor-not-allowed disabled:opacity-40"
          >
      ...
      {/* ── Members side panel removed (handled by AdminNavbar) ─── */}
    </div>
  );
}
